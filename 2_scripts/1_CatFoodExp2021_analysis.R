...
raw_surv <- ggplot(data=surv_avg, aes(x=MismTreat, y=probs))+
  ...
  geom_jitter(data=surv_probs, aes(col=PhotoTreat), alpha=0.3, size=2, height=0, width=0.25)+
...
p_surv
# ggsave(filename="_results/Survival_wpred.png", plot=p_surv, device="png", width=200, height=150, units="mm", dpi="print")
...
d_pupa <- d %>% mutate(PhotoTreat=gsub("(\\w+)Day.+","\\1",Treatment), MismTreat=gsub("\\w+(Day.+)","\\1",Treatment), PupaWeight=PupaWeight*1000) %>%
...
raw_weight <- ggplot(data=weight, aes(x=MismTreat, y=PupaWeight, col=PhotoTreat))+
  scale_colour_manual(values=c("grey27", "dodgerblue4"))+
  geom_jitter(data=d_pupa, aes(col=PhotoTreat), alpha=0.3, size=2, height=0, width=0.25)+
  geom_point(size=5)+
  geom_errorbar(aes(ymax = PupaWeight+se, ymin=PupaWeight-se), width=0.3) +
...
p_weight
# ggsave(filename="_results/PupWeight_wpred.png", plot=p_weight, device="png", width=200, height=150, units="mm", dpi="print")
...
#-----------------------------------
RelFit <- merge(glm.fit[,c("TubeID", "MismTreat", "survpred")], lm.fit[,c("TubeID", "MismTreat", "pred")], by=c("TubeID", "MismTreat"))
...
head(RelFit_means)
# write.csv(RelFit_means, file="_results/RelFitness.csv", row.names=F)

p_relfit <- ggplot(data=RelFit, aes(x=MismTreat, y=rel)) +
  geom_jitter(size=2, alpha=0.3, height=0, width=0.25)+
  ...
  geom_text(data=RelFit_means,aes(label=samplesize, y=rel+0.15), col="black", size=5, fontface="bold")+ # number of caterpillars
  ...
  theme(axis.title.y=element_text(size=18, vjust=2), axis.title.x=element_text(size=18, vjust=-0.5),
        axis.text=element_text(size=16), legend.text = element_text(size=16), legend.title=element_text(size=17))
p_relfit
# ggsave(filename="_results/FitnessCurve.png", plot=p_relfit, device="png", width=200, height=150, units="mm", dpi="print")
...
# Timing data ####
d_tim <- d %>% 
  select(YearCatch, YearHatch, Treatment, TubeID, ClutchID, CaterpillarID, HatchAprilDay, PupationAprilDay, PupaWeight) %>%
  filter(!is.na(PupationAprilDay)) %>%
  mutate(PhotoTreat=gsub("(\\w+)Day.+","\\1",Treatment), MismTreat=gsub("\\w+(Day.+)","\\1",Treatment), Event=1) %>% # all included have pupated
  mutate(Treatment=as.factor(Treatment), PhotoTreat=as.factor(ifelse(PhotoTreat=="Chang", "Changing", "Constant")), MismTreatf=as.factor(MismTreat), TubeID=as.factor(TubeID)) %>%
  mutate(Treatment=factor(Treatment, levels=c("ChangDay-4", "ChangDay-3", "ChangDay-2", "ChangDay-1", "ChangDay0", "ChangDay+1", "ChangDay+2", "ChangDay+3", "ChangDay+4",
                                              "ChangDay+5",  "ConstDay-4", "ConstDay-2", "ConstDay0", "ConstDay+2", "ConstDay+4")), 
         MismTreatf=factor(MismTreat, levels=c("Day-4", "Day-3", "Day-2", "Day-1", "Day0", "Day+1", "Day+2", "Day+3", "Day+4", "Day+5")),
...
p_devtime <- ggplot(data=devtime, aes(y=larv_devtime, x=MismTreat, col=PhotoTreat)) +
  scale_colour_manual(values=c("grey27", "dodgerblue4"))+
  geom_jitter(data=d_tim, size=2, alpha=0.3, height=0, width=0.25)+
  geom_point(size=5)+
...
p_larvdev
# ggsave(filename="_results/LarvDevTime_wpred.png", plot=p_larvdev, device="png", width=200, height=150, units="mm", dpi="print")
