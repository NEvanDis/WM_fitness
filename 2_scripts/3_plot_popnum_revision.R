...
p_time <- p_popnr + geom_hline(yintercept=30, linetype="dashed", size=1) +
  geom_line(data=timing1, aes(x=YearHatch, y=Mismatch.Tree2, group=grp), col="orangered2") + #"grey50"
  geom_point(data=timing1, aes(x=YearHatch, y=Mismatch.Tree2), size=2, col="orangered2", alpha=0.9)+ #add mismatch, col=""grey50""
...
p_time
orangered2ggsave(filename="_results/PopNrs_wmismatch_rev.png", plot=p_time , device="png", width=280, height=150, units="mm", dpi="print")
