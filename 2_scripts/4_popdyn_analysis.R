...
# Eggs
d_eggs <- read_csv("1_data/1993-2021_Field_Eggs_deposition.csv")
head(d_eggs)
...
p_eff <- ggplot(data=d, aes(x=Mismatch.mean, y=resid)) + # plot Mismatch effect, filter(d, Mismatch.mean>-21 & Mismatch.mean<0)
  ...
  geom_point(size=5, shape=21, fill="grey27", col="black", alpha=0.7) +
  geom_line(aes(y=pred), size=1, col="red3")+
  ...
  theme(strip.text=element_text(size=18), axis.text = element_text(size=15), axis.title=element_text(size=18))
p_eff
# ggsave(filename="results/Mismatch_effect.png", plot=p_eff , device="png", width=220, height=150, units="mm", dpi="print")
